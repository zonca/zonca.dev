...
You can also use the terminal to generate images using the **Nano Banana Pro** (Gemini 3 Pro Image) model.

![Nano Banana Pro generated image](img/nano-banana-pro-hero.png)

To generate an image, use the `/image` command followed by your prompt:

```bash
/image "A futuristic city with flying cars" --model nano-banana-pro
```
