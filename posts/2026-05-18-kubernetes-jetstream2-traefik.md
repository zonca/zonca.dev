...
This guide assumes you already have a running cluster — see [Deploy Kubernetes on Jetstream2](/posts/kubernetes-jetstream2-magnum) if you need to create one first. Make sure `KUBECONFIG` is set and `kubectl get nodes` works.
...
With Traefik installed and DNS configured, continue to [Deploy JupyterHub](/posts/kubernetes-jetstream2-jupyterhub) or [Setup HTTPS with cert-manager](/posts/kubernetes-jetstream2-certmanager).
